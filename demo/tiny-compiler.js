// Source: https://github.com/mgechev/tiny-compiler/blob/master/tiny.js
...
const compile = ast => {
  ...
  const compileOp = ast => `(${ast.expr.map(compileExpr).join(' ' + opMap[ast.val] + ' ')})`;

  const compileExpr = ast => ast.type === Num ? compileNum(ast) : compileOp(ast);
